...
abstract class Model
{
    ...
    /** @var string $entityB database table */
    protected $entityB;
    ...
    public function findCat(?string $terms = null, ?string $params = null, string $columns = "*")
    {
        if ($terms) {
            $this->query = "SELECT {$columns}, a.uri AS subcategoryby, b.uri AS bycategory, b.title AS name, b.cover AS photo FROM {$this->entity} a INNER JOIN {$this->entityB} b WHERE {$terms}";
            parse_str($params, $this->params);
            return $this;
        }

        $this->query = "SELECT *, a.uri AS subcategoryby, b.uri AS bycategory, b.title AS name, b.cover AS photo FROM subcategory a INNER JOIN servicecategory b WHERE b.id = a.idcategory";
        return $this;
    }
